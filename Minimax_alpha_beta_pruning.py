...
def main():
    ...
    while check_terminal(board) == -1:
        ...
        board, val = minimax(board, 'min', -1000, 1000)
        end_time = time.time()
    ...
    if flag == 'min':
        ...
        for a in actions:
            ...
            if next_state is None:
                continue
            garbage, temp = minimax(copy.deepcopy(next_state), 'max', alpha, beta)
            if temp < min_val:
                min_val = temp
                state = next_state
            if min_val <= alpha:
        ...
        for a in actions:
            ...
            if next_state is None:
                continue
            garbage, temp = minimax(copy.deepcopy(next_state), 'min', alpha, beta)
            if temp > max_val:
                max_val = temp
                state = next_state
            if max_val >= beta:
